...
\Abstract {
This paper briefly reviews the panel unit root tests implemented in \proglang{R}. All the presently available functions are implemented in either the \pkg{plm} package \citep{croissantmillo2008} or the \pkg{punitroots} package \citep{lupi2009, costantinilupi2011}. The performance of the tests is compared using some Monte Carlo experiments carried out under a fairly realistic Data Generating Process. The practical application of the different tests is illustrated using international unemployment data.
}
...
\SweaveOpts{prefix.string=/home/claudio/Documents/Papers/LupiRJ2010/Docs/Fig,keep.source=TRUE}
...
In this paper we offer a brief survey of panel unit root testing with \proglang{R}. In fact, only two \proglang{R} packages, namely \pkg{plm} \citep{croissantmillo2008} and \pkg{punitroots} \citep{lupi2009, costantinilupi2011}, implement panel unit root tests. In particular, seven panel unit root tests are implemented (three in \pkg{plm} and four in \pkg{punitroots}), plus one stationarity test (in \pkg{plm}). It is fair to say that \pkg{plm} is a nice package on panel linear models, only marginally concerned with panel unit root testing: nevertheless it has been the first package in \proglang{R} dealing with that issue. 
...
A plot of the series is offered in Figure~\ref{figureUnempl}. The object \code{unempseries} is a multiple quarterly time series that spans the period $\Sexpr{start(unempseries)[1]}$q$\Sexpr{start(unempseries)[2]}$--$\Sexpr{end(unempseries)[1]}$q$\Sexpr{end(unempseries)[2]}$, although some individual series are shorter. 
